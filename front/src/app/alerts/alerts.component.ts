...
export class AlertsComponent implements OnInit {
  @ViewChild(CardAlertsComponent, {static: true}) cardAlerts: CardAlertsComponent;
  alerts: Alert[];
  probes: Probe[];
  ...
  addAlert(alert: Alert) {
    const alertPos = this.alertPos(alert);
    if (alertPos < 0) {
  ...
  alertExists(alert: Alert): boolean {
    for (const cAlert of this.alerts) {
  ...
  orderedAlertPos(alert: Alert): number {
    ...
    const alerts = this.orderedAlerts.alerts[alert.identifier][alert.probe];
    if (alerts.length === 0) {
      return -1;
    }
    for (let i = 0; i < alerts.length; i++) {
      const cAlert = alerts[i];
  ...
  pushSiteNotification(alert: Alert) {
    const notif = this.notifService.alert(capitalizeFirstLetter(alert.status), alert.notification);
  ...
  pushNotification(title: string, body: string) {
    const notif = this.browserNotifSvc.create(
      title,
      {body: body}
    );
    notif.subscribe(
  ...
  pushAlertNotification(alert: Alert) {
    const alertExists = this.alertExists(alert);
  ...
  addOrderedAlert(alert: Alert) {
    const alertPos = this.orderedAlertPos(alert);
    if (alertPos >= 0) {
  ...
  hasShowableAlerts(identifier: string): boolean {
    ...
    let count = 0;
    const probes = this.orderedAlerts.alerts[identifier];
    for (const probe of Object.keys(probes)) {
      const alerts = this.orderedAlerts.alerts[identifier][probe];
  ...
  gridShowable(): boolean {
    ...
    const ids = this.orderedAlerts.identifiers;
    for (const id of ids) {
  ...
  getOrderedAlerts(): void {
    this.alertService.getOrderedAlerts()
      .subscribe(orderedAlerts => {
        ...
        const identifiers = this.orderedAlerts.alerts;
