...
const DefaultMessageTpl = `[alertmanager]({{ .NotifierURL }}) -- **{{ .Name }}**: {{ .Annotations.summary }}
Alert generated by {{ .GeneratorURL }}
**Details**: {{ .Annotations.description }}`

const DefaultNotificationTpl = `{{ .Name }} - {{ .Identifier }} - {{ .Probe }}`

type Probe struct {
	// A unique identifier for this receiver.
	Name     string    `yaml:"name" json:"name"`
	DocUrl   string    `yaml:"doc_url" json:"doc_url"`
	Template *Template `yaml:"template,omitempty" json:"-"`
...
func (c *Probe) UnmarshalYAML(unmarshal func(interface{}) error) error {
	...
	if c.Template.Message == "" {
		c.Template.Message = DefaultMessageTpl
	}
	if c.Template.Notification == "" {
		c.Template.Notification = DefaultNotificationTpl
	}
